...
from settings.settings import (
    ...
    IF_THRESHOLD_PATH,
    LSTM_MODEL_PATH,
    LSTM_TOKEN_MODEL_PATH,
    LSTM_TOKEN_TRANSFORMER_PATH,
...
def load_lstm_token_bundle():
    if not LSTM_TOKEN_MODEL_PATH.exists():
        return None
    try:
        import torch
    except Exception as e:
    ...
    checkpoint = torch.load(LSTM_TOKEN_MODEL_PATH, map_location="cpu")
    bundle = {"model": None, "config": {}, "transformer": None}

    transformer = None
    if LSTM_TOKEN_TRANSFORMER_PATH.exists():
        transformer = joblib.load(LSTM_TOKEN_TRANSFORMER_PATH)
    ...
    if isinstance(checkpoint, dict) and "state_dict" in checkpoint:
        ...
        if vocab_size is None and transformer is not None:
            vocab_size = len(transformer.template_list_ or [])
        if vocab_size is None:
            raise ValueError("LSTM token checkpoint missing vocab_size")
        embedding_dim = config.get("embedding_dim", 32)
        hidden_size = config.get("hidden_size", 64)
        num_layers = config.get("num_layers", 1)
        dropout = config.get("dropout", 0.0)
        ...
        raise ValueError("Unsupported LSTM token checkpoint format")
...
async def lifespan(app: FastAPI):
    ...
    __main__.SequenceTokenTransformer = SequenceTokenTransformer
    ...
    try:
        ...
        app.state.lstm_token_bundle = load_lstm_token_bundle()
        settings = Settings()
        ...
        app.state.lstm_token_bundle = None
...
@app.post("/forward/lstm-token", response_model=PredictionResponse)
async def forward_prediction_lstm_token(
    request: PredictionRequest,
    session: Session = Depends(get_session),
    current_user: Optional[str] = Depends(get_current_user),
):
    ...
    if app.state.lstm_token_bundle is None:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="LSTM token model was not loaded",
        )
    ...
    transformer = None
    if isinstance(app.state.lstm_token_bundle, dict):
        transformer = app.state.lstm_token_bundle.get("transformer")

    if transformer is None:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="LSTM token transformer was not loaded",
        )
    ...
    if windows_df.empty:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="No valid windows for LSTM token inference",
        )
    if "target_window" not in windows_df.columns:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="LSTM token windows must include target windows for scoring",
        )
    ...
    config = {}
    if isinstance(app.state.lstm_token_bundle, dict):
        model = app.state.lstm_token_bundle.get("model")
        config = app.state.lstm_token_bundle.get("config", {}) or {}

    if model is None:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="LSTM token model object was not loaded",
        )

    scoring = config.get("scoring", "nll_max")
    score_threshold = float(
        config.get(
            "anomaly_score_threshold",
            config.get("anomaly_ratio_threshold", 0.5),
        )
    )
    device = config.get("device", "cpu")
    if str(device).startswith("cuda") and not torch.cuda.is_available():
    ...
    targets = torch.tensor(
        windows_df["target_window"].tolist(),
        dtype=torch.long,
        device=device,
    )
    ...
    with torch.no_grad():
        ...
        if scoring != "nll_max":
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Unsupported LSTM token scoring: {scoring}",
            )
        log_probs = torch.log_softmax(logits, dim=-1)
        nll = -log_probs.gather(dim=-1, index=targets.unsqueeze(-1)).squeeze(-1)
        anomaly_score = nll.max().item()

    prediction_lstm = "Anomaly" if anomaly_score >= score_threshold else "Normal"
...
async def health_check():
    ...
    return {
        ...
        "lstm_token_model_loaded": app.state.lstm_token_bundle is not None,
    }
