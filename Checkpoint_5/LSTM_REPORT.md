...
2. LSTM для next‑event prediction (обучение на нормальных блоках) + порог по валидации
...
## 8. Результаты
Полученные метрики (при threshold=0.09, подобранном по val):
- `precision` = 0.820 (большая часть срабатываний действительно аномалии)
- `recall` = 0.740 (около 34% аномалий пропущено)
