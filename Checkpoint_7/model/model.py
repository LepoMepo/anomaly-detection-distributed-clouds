...
class MultiHeadAttentionBlock(nn.Module):
    ...
    @staticmethod
    def attention(query, key, value, mask, dropout: nn.Dropout, key_padding_mask=None):
        d_k = query.shape[-1]
        ...
        if key_padding_mask is not None:
            ...
            padding_mask = padding_mask.expand(-1, query.shape[1], query.shape[2], -1)
            attention_scores.masked_fill_(padding_mask, -1e9)
    ...
    def forward(self, q, k, v, mask, key_padding_mask=None):
        query = self.w_q(q)
        key = self.w_k(k)
        value = self.w_v(v)
        ...
        x = MultiHeadAttentionBlock.attention(query, key, value, mask, self.dropout, key_padding_mask)
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, src_mask, src_key_padding_mask=None):
        x = self.residual_connections[0](x, lambda x: self.self_attention_block(x, x, x, src_mask, src_key_padding_mask))
